...
// Implement a simple clustering method
function simpleCommunityDetection(nodes, links) {
    const clusters = {};
    let clusterId = 0;
    ...
    nodes.forEach(node => {
        if (!node.cluster) {
            ...
            while (queue.length > 0) {
                ...
                links.filter(l => l.source.id === current.id || l.target.id === current.id)
                     .map(l => l.source.id === current.id ? l.target : l.source)
                     .forEach(neighbor => {
                         if (!neighbor.cluster) {
...
nodes = simpleCommunityDetection(nodes, links);
...
const node = svg.append("g")
    ...
    .data(nodes)
    .join("circle")
    .attr("r", d => d.group === 1 ? 8 : 6) // Larger for institutions
    .attr("class", "node")
    .attr("fill", d => clusterColor(d.cluster)) // Use cluster color for fill
    .attr("stroke", d => groupColor(d.group)) // Use group color for stroke to differentiate node types
    .call(d3.drag()
        .on("start", dragstarted)
        .on("drag", dragged)
        .on("end", dragended));

// Add labels, but only for institutions for clarity
const label = svg.append("g")
    .selectAll("text")
    .data(nodes.filter(d => d.group === 1)) // Only label institutions
    .enter().append("text")
    .attr("x", d => d.x)
    .attr("y", d => d.y + 15) // Position below the node
    .text(d => d.id)
...
node.on("mouseover", function(event, d) {
    tooltip.transition()
        ...
        .style("opacity", .9);
    tooltip.html(`${d.id}
Type: ${['Institution', 'Activity', 'Collaboration', 'Stakeholder'][d.group - 1]}
Cluster: ${d.cluster}`)
...
simulation.on("tick", () => {
    ...
    label
        .attr("x", d => d.x)
        .attr("y", d => d.y + 15); // Keep labels below nodes
});
...
svg.append("text")
    ...
    .text("Interactive Cluster Network of Institutional Connections");

// Legend for node types and clusters
const legend = svg.selectAll(".legend")
    .data([...new Set(nodes.map(d => d.cluster))].sort((a, b) => a - b))
    .enter().append("g")
    .attr("class", "legend")
    .attr("transform", (d, i) => `translate(20,${30 + i * 20})`);
...
const groupLegend = svg.selectAll(".groupLegend")
    .data([1, 2, 3, 4])
    .enter().append("g")
    .attr("class", "groupLegend")
    .attr("transform", (d, i) => `translate(20,${30 + i * 20 + legend.nodes().length * 20})`);

groupLegend.append("rect")
    .attr("width", 18)
    .attr("height", 18)
    .style("stroke", d => groupColor(d))
    .style("fill", "none");
...
groupLegend.append("text")
    ...
    .attr("y", 9)
    .attr("dy", ".35em")
    .text(d => ['Institution', 'Activity', 'Collaboration', 'Stakeholder'][d - 1]);